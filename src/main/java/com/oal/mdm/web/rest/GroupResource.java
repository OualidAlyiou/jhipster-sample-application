...
import com.oal.mdm.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.oal.mdm.domain.Group}.
 */
@RestController
@RequestMapping("/api")
public class GroupResource {
  ...
  public ResponseEntity<Group> createGroup(@Valid @RequestBody Group group)
    ...
    if (group.getId() != null) {
      throw new BadRequestAlertException(
        "A new group cannot already have an ID",
        ENTITY_NAME,
        "idexists"
      );
    }
  ...
  public ResponseEntity<Group> partialUpdateGroup(
    ...
    log.debug(
      "REST request to partial update Group partially : {}, {}",
      id,
      group
    );
    if (group.getId() == null) {
