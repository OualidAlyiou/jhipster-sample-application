...
export class GroupService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/groups");
  ...
  create(group: IGroup): Observable<EntityResponseType> {
    return this.http.post<IGroup>(this.resourceUrl, group, {
      observe: "response",
    });
  }
  ...
  update(group: IGroup): Observable<EntityResponseType> {
    return this.http.put<IGroup>(
      `${this.resourceUrl}/${getGroupIdentifier(group) as number}`,
      group,
      { observe: "response" }
    );
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IGroup>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IGroup[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addGroupToCollectionIfMissing(
    groupCollection: IGroup[],
    ...groupsToCheck: (IGroup | null | undefined)[]
  ): IGroup[] {
    const groups: IGroup[] = groupsToCheck.filter(isPresent);
    if (groups.length > 0) {
      const groupCollectionIdentifiers = groupCollection.map(
        (groupItem) => getGroupIdentifier(groupItem)!
      );
      const groupsToAdd = groups.filter((groupItem) => {
        const groupIdentifier = getGroupIdentifier(groupItem);
        if (
          groupIdentifier == null ||
          groupCollectionIdentifiers.includes(groupIdentifier)
        ) {
          return false;
        }
